...
- スケールバー・矢印: Jimp（**尺度も指す先も利用者が示したものをそのまま描く**。下記）
...
規則ベースへ戻す。清書に失敗したときは**訳すことだけ**をもう一度頼み、それでも日本語が
残ったら**編集は走らせずに止める**（D-033）。編集モデルは日本語を汲まないので、
走らせても親画像がほぼそのまま返るだけで、45秒が捨てられる。生成は止めない——
多言語の生成モデルは日本語のプロンプトでも主題どおりに描く（実測）。
LLMを設定していない場合、生成・回転・切り抜き・明るさ・ロゴタイプ・範囲の消去・
背景の透明化は規則ベースで使えるが、**日本語での生成編集はできない**。有効時はツール選択に加えて、系譜の指示から描画する文字列
（製品名など）を推定し、雑な指示を画像モデル向けの英語プロンプトへ書き直す。
書き直しは翻訳と曖昧さの解消に限り、頼まれていない様式は足させない。
**全体を作り替える依頼かどうかも、ここで答えさせる**（D-023）。「シンプルにして」
「抽象化して」の直後に「既存の要素をすべて保て」と書くと自己矛盾し、モデルは保存の方に
従って絵がほとんど変わらない（実測）。**保存を求める文は条件形で書く**——`Do not change anything the instruction does not ask
you to change.`。絶対形（`Preserve all existing visual elements...`）は作り替えの依頼と
正面から矛盾し、モデルは保存の方に従う（実測2回）。条件形なら**どんな指示とも矛盾しない**
ので、**範囲の判定を外しても壊れない**。

そのうえで、作り替えの依頼には「描き直してよい」と強く伝える。判定は利用者の言い回しと
**実際にモデルへ渡る書き直し文**の両方に当て、外したときにどちらを直せばよいか分かるよう、
**決めた側（AI か規則か）を画面に出す**。
...
気に入らない版は「この版をアーカイブ」で候補の並びとグラフから外せる（D-032）。**消さない**——
アーカイブしたことは表明として残り、いつでも戻せる。グラフの右上と候補の下に
「アーカイブ N 件を表示」が出るので、隠したものは在ることが分かる。
系譜の途中の版を隠したときは、**子をその上の見えている版へ細い点線で繋ぎ直す**ので、
どこから来たかは辿れる。会話をまるごと消すほうは従来どおり。
...
**できた順に出す。** 生成は直列に 1 本ずつ流すので、4 枚だと 10 分近くかかる。
まとめて返るのを待たず、**1 枚できるたびに画面へ出す**（待っている間はグラフを取り直す）。
進み具合は「生成中… 2/4 枚」として、まだ会話が無いときは真ん中の面にも出す。
流している間は指示欄を伏せる——送れないのに書けると、送ったつもりになる。
...
候補を複数選んだ場合は「指示のAI解釈」で**方向の違うコンセプト**を作る。
コンセプトは日本語の案名として候補画像に表示し、画像モデルへの指示は英語にする。
無効な場合や必要な数のコンセプトを作れなかった場合は生成前に理由を返す。
新しい図案は提案できるが、利用者の明示条件は守る。**足した内容は記録に残る**——
実行された全文と利用者の生の言葉は版ごとに別々に残るので、
...
**矢印も、どこを指すかはこちらでは決めない。** 「矢印を引く」から画像の上でドラッグすると、
その位置が引数になる。位置は**画像の大きさに対する％**で残すので、後から縮めても指す所が
変わらない。文字を添えれば矢の根元に置く（終点に置くと、指している物に重なる）。

```
{"text":"grain boundary","x1":18,"y1":17,"x2":67,"y2":60}
```
...
| `annotated` | Jimp（ロゴタイプ・スケールバー・矢印） | 元の画素は残したまま、確定的な描画を重ねる |
...
デスクトップ版は**ポートを OS に選ばせる**（D-024）。0 を渡せば空きが保証されるので、
他のアプリと取り合いにならない。割り当てられた番号は、サーバが起動時に出す行から読む。

```
PROVision server: http://127.0.0.1:60464  data=…
```

聞き取れなかったときの受け皿として、8788 から 8792 まで順に試す道も残してある。
どちらの道でも、`/api/health` が `{"app":"provision"}` を返したところだけを自分の
サーバとみなす。**200 が返っただけでは信用しない**——他人の画面 HTML が 200 で
返ってくることが実際にあった。

ブラウザでの開発（`pnpm dev`）は従来どおり 8788 固定（vite のプロキシ先）。

### 保存先

**成果物**（グラフの JSON-LD と画像）の既定は **`~/Documents/PROVision`**。
Application Support に隠すと、利用者が中身を見にいけない。この道具の成果物は
書き出したファイルそのものなので、手の届く場所に置く。

**設定**（`ai-planner.json` / `identity.json` / `policy.json`）は分けて
**Application Support** に置く（`PROVISION_CONFIG_DIR`）。Documents は macOS の
TCC 保護対象で、**アプリの署名が変わると過去に与えた許可が無効化される**。
リリースのたびに署名は変わるので、更新直後のサイドカーは Documents を読めない
状態で起動しうる。そのとき `existsSync` は「拒否された」ではなく `false` を返すため、
「設定がまだ無い」と区別できない——**AI モデルの登録が更新のたびに消えたように見え、
その状態で登録し直すと元の内容ごと上書きされていた**。設定は利用者が直接開くもの
ではないので、TCC 保護外へ逃がす（Graphium が同じ理由で Documents から移している）。

置き場を分ける前の設定は、起動時に一度だけ複製して引き継ぐ。**移動ではなく複製**
なので、古い版へ戻しても設定は残る。移行先に同名があれば必ず飛ばす。
